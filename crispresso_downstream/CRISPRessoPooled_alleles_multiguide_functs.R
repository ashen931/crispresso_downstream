...
### Functions:
#     get_CRISPRessoPooled_allele_tbs()
#     get_pooled_allele_frequency_table() 
#         get_pooled_run_info()
#         get_pooled_run_table()
#         reverse_complement() 
#         get_pool_plot_allele_table_from_file()
...
###### get_CRISPRessoPooled_allele_tbs() #############################################################################
# This function takes a list of CRISPRessoPooled file names within the current working
# directory, extracts summary allele tables from each file, and saves the tables in the
# current directory.
#
# CALLS HELPERS: get_pooled_allele_frequency_table(), adjust_percent_frequency()
# CALLED IN: CRISPResso2_downstream.R
# ARGUMENTS:  crispresso2_pooled_files = list of sample directories (ex. list("./CRISPRessoPooled_on",...))
#             percent_freq_cutoff = minimum percent frequency an allele must appear in at least 1 
#                                   sample in order to be included in the final output table
#             dataID = string idenitfier in the sample names that identifies a column as a column of 
#                      allele frequencies
#             percent_freq_cutoff = minimum percent frequency an allele must appear (for ANY amplicon within
#                                   the CRISPRessoPooled run) to be included in the collapsed allele table 
#             noSub = boolean/logical indicating whether or not substitutions are counted as real indels
#                     in the analysis
# OUTPUT: none (directly saves allele tables to .csv files in current directory)
get_CRISPRessoPooled_allele_tbs <- function(crispresso2_pooled_files,
                                            dataID,
                                            percent_freq_cutoff,
                                            noSub){
  
  #generate analysis log
  #sink("CRISPRessoPooled_collapsing_log.txt", append=FALSE, split=TRUE)
  
  cat("CRISPRessoPooled_allele_collapsing_log\n",
      paste(Sys.time(), "\n", sep = ""),
      paste(getwd(), "\n", sep = ""),
      "dataID: ", dataID, "\n",
  ...
  for(n in seq(1,length(crispresso2_pooled_files))){
    
    #set working directory to correct pooled directory
    cat("***************************************************************************\n",
        "CRISPRessoPooled run: ", crispresso2_pooled_files[n], "\n")
    setwd(crispresso2_pooled_files[n])
    
    #get csv_prefix for saving file
    csv_prefix <- sub("./CRISPRessoPooled_on_", "", crispresso2_pooled_files[n])
    
    #select the list of CRISPresso2 output directories (may be 2-3 directories deep) 
    ...
    #  data table
    final_allele_table <- get_pooled_allele_frequency_table(crisp_out_dir, sample_dirs_list, 
...
#
# CALLS HELPERS: get_pool_plot_allele_table_from_file(), select_alleles_for_plotting(),
...
get_pooled_allele_frequency_table <- function(crisp_out_dir, sample_dirs_list, percent_frequency, dataID, noSub){
  ...
  for(sample_dir in sample_dirs_list){ #loop through directories in sample_dirs_list
    ...
    crispresso_run_info <- get_pooled_run_info(run_log_string[1])
    ...
    if(grepl("ERROR", run_log_string)){
      ...
      #If the desired allele is not found among the reference sequences, cat "XXX allele not found."
      ...
      indel_table_unique <- get_pooled_run_table(sample_dir, crispresso_run_info, dataID, noSub)
...
#
# CALLS HELPERS: NA
# CALLED IN: get_pooled_allele_frequency_table()
...
get_pooled_run_info <- function(run_log_string){
  
  #extract the guide sequence
  guide_seqs <- str_extract(run_log_string[1],"-g [ACGT[:punct:]]{6,} --") %>% 
    str_remove( "-g ") %>% str_remove(" --")
  
  #if more than one guide, split the guides into two
  if(grepl(",", guide_seqs)){
    guide_seqs <- strsplit(guide_seqs , split = ",")
  }
  
  #find the numeric index (order) of the desired allele in the amplicon name & sequeunce list
  ref_names <- str_extract(run_log_string[1],"--name [[:alnum:][:punct:]\\s]{1,} -g") %>%
    str_remove("--name ") %>% str_remove(" -g") %>% strsplit(split = ",")
  ref_name <- ref_names[[1]]
  
  list_ref_seqs <- str_extract(run_log_string[1],"-a [ACGTatcg[:punct:]]{1,} -") %>%
    str_remove("-a ") %>% str_remove(" -") %>% strsplit(split = ",")
  #extract the amplicon sequence
  ref_seq <- list_ref_seqs[[1]]
  ...
  #get run info
  run_info <- data.frame(guide_seq = unlist(guide_seqs), ref_seq = rep(ref_seq, n_guides), 
                         ref_name = rep(ref_names,  n_guides),
                         q_window = rep(q_window, n_guides), 
                         qw_center = rep(qw_center,  n_guides), 
                         plot_window_size = rep( plot_window_size * 2, n_guides))
...
#                     CRISPResso run
get_pooled_run_table <- function(sample_dir, crispresso_run_info, dataID, noSub){
  ...
  for(n in seq(1, nrow(crispresso_run_info))){
    ...
    allele_table <- get_pool_plot_allele_table_from_file(sample_dir, guide_seq)
    
    #cat information to check everything is correct
    ...
    colnames(ranked_indel_table) <- sub("_percent_reads", 
                                        paste("__", guide_seq, sep = ""),
                                        colnames(ranked_indel_table))
    colnames(ranked_indel_table) <- sub("_reads", 
                                        paste("_reads__", guide_seq, sep = ""),
                                        colnames(ranked_indel_table))
    
    cat("*************", "\n")
    cat("Frequency_unedited_alleles: ", "\n")
...
get_pool_plot_allele_table_from_file <- function(sample_dir, guide_seq){
  ...
  read_name_percent <-paste(sub("CRISPResso_on_", "", sample_handle[[1]][length((sample_handle[[1]]))]),"_percent_reads", sep = "")
  read_name_reads <-paste(sub("CRISPResso_on_", "", sample_handle[[1]][length((sample_handle[[1]]))]),"_reads", sep = "")
...
get_indel_columns <- function(ranking_table, cut_minus1, cut_index_adjustment, ref_seq, q_window, noSub){
  ...
  q_window_start <- cut_minus1 + 1 - q_window #- q_window #/2 + 1 ADDED
  #q_window_start <- cut_minus1 
  q_window_end <- cut_minus1 + q_window #/2
  
  #cat q_window start and end (for debugging purposes)
...
collapse_duplicate_indels <- function(ranked_indel_table, dataID){
  ...
  #get the first & last column indices of allele frequency & read data
  ...
  while (num_row < nrow(ranked_indel_table)){
    ...
    if((ranked_indel_table$indel[num_row] %in% duplicate_indels_list)){
      ...
      if (length(allele_freq_cols) == 2){ #if there are allele frequency+reads columns
...
save_to_csv <- function(table_to_save, save_directory, save_name){
  ...
  #write table_to_save to csv file
...
adjust_percent_frequency <- function(final_allele_table, dataID){
  ...
  #get names of columns containing sample allele frequencies
  data_cols <- grep("reads", grep(dataID, names(final_allele_table), value = TRUE), 
                    invert = TRUE, 
                    value = TRUE)
  
  #sum data columns to get total allele frequency
  new_allele_total_perc <- colSums(select(final_allele_table, data_cols), na.rm = TRUE)
  
  #re-normalize each allele frequency so that the total frequency is 100%
  for(n in data_cols){
